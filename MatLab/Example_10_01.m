...
% User subfunctions required: rates, terminate, find_extrema
% ------------------------------------------------------------------------
...
[max_altitude,imax,min_altitude,imin] = find_extrema(altitude);
...
function [max_val, max_idx, min_val, min_idx] = find_extrema(data)
    % This function finds the local maxima and minima in a vector.
    max_val = [];
    min_val = [];
    max_idx = [];
    min_idx = [];

    % Loop through data to find local maxima and minima
    for i = 2:length(data)-1
        if data(i) > data(i-1) && data(i) > data(i+1)
            max_val = [max_val; data(i)];
            max_idx = [max_idx; i];
        elseif data(i) < data(i-1) && data(i) < data(i+1)
            min_val = [min_val; data(i)];
            min_idx = [min_idx; i];
        end
    end
end %find_extrema
